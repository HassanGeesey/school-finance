...
**Review pass (commit `bfd536f`, post-ticket):** code review flagged that the print statement included the action buttons and that the account page lacked a credits line. The nav/action/alerts area in `students/account.html` is now `no-print` (the statement header + finance table still print); a Credit card listing the account's `Credit` rows was added to `fees/_account_finance.html`; the `AccountView` docstring now states `credits_cents` is total carried credit (the per-month Credit column shows consumption). The billing test factories (`make_billed_student`/`add_payment`/`add_credit`) moved into `tests/helpers.py` (they were duplicated across `test_fees_account.py`, `test_reports_service.py`, `test_arrears_service.py`).
