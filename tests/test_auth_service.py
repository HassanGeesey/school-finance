...
from app.models import AuthSession, User, UserRoles
...
def test_setup_first_admin_creates_admin_with_hashed_password(auth, session):
    user = auth.setup_first_admin(name="Head Teacher", username="admin", password=PASSWORD)
...
def test_setup_first_admin_rejects_a_second_admin(auth, session):
    ...
    with pytest.raises(SetupNotAvailable):
        auth.setup_first_admin(name="B", username="admin2", password=PASSWORD)
    assert session.query(User).count() == 1


def test_setup_first_admin_requires_name_and_username(auth):
    with pytest.raises(AuthError):
        auth.setup_first_admin(name="", username="admin", password=PASSWORD)
    with pytest.raises(AuthError):
        auth.setup_first_admin(name="   ", username="admin", password=PASSWORD)
    with pytest.raises(AuthError):
        auth.setup_first_admin(name="A", username="", password=PASSWORD)


def test_setup_first_admin_requires_a_password(auth):
    with pytest.raises(AuthError):
        auth.setup_first_admin(name="A", username="admin", password="")
...
def test_authenticate_rejects_a_disabled_user(auth, session):
    user = auth.setup_first_admin(name="A", username="admin", password=PASSWORD)
