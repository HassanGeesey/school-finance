...
- **Active feature:** `multi-school` — the tenant layer on the cloud path. Tickets and spec live under `.scratch/multi-school/`. The `fee-billing-rework` feature is implemented; ignore `.scratch/school-finance/` (deprecated; its leftover tickets 22–24 are not to be worked on).
